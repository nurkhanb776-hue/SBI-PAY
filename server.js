...
const getNextUserId = users => {
  const numericIds = (Array.isArray(users) ? users : [])
...
const nextUserId = async database => {
  const users = database.collection('users');
  const latestUser = await users.aggregate([
    {$project: {numericId: {$convert: {input: '$userId', to: 'long', onError: 0, onNull: 0}}}},
    {$match: {numericId: {$gte: 20000}}},
    {$sort: {numericId: -1}},
    {$limit: 1}
  ]).next();
  const latestNumericId = Number(latestUser?.numericId || 20000);
  const counters = database.collection('counters');
  const counter = await counters.findOneAndUpdate(
    {_id: 'users'},
    {$max: {value: latestNumericId}},
    {upsert: true, returnDocument: 'after'}
  );
  const currentValue = Number(counter.value?.value || latestNumericId);
  const nextValue = Math.max(20001, currentValue + 1);
  await counters.updateOne({_id: 'users'}, {$set: {value: nextValue}}, {upsert: true});
...
const ensureRequiredFallbackUsers = data => {
  const requiredUsers = [
    {userId: '20000', username: 'Admin1', phone: '8837022561', passwordHash: 'placeholder', inviteCode: 'SBI20000', ownerCode: '', upiId: 'masudmiah09@naviaxis', bankAccount: '41029268462', bankName: 'STATE BANK OF INDIA', bankHolder: 'NUR SALAM ALI', bankIfsc: 'SBIN0005807', isActive: true, balance: 25000, depositBalance: 25000, packageName: 'Primary Admin', packageAmount: 25000, signupBonusAmount: 399, walletLimit: 100000, status: 'enabled', createdAt: new Date().toISOString()},
    {userId: '20001', username: 'demo_member', phone: '8837022562', passwordHash: 'placeholder', inviteCode: 'SBI20001', ownerCode: 'SBI20000', upiId: '8837022562@upi', bankAccount: '450123987654', bankName: 'HDFC BANK', bankHolder: 'Demo Member', bankIfsc: 'HDFC0001234', isActive: true, balance: 399, depositBalance: 399, packageName: 'Free 399', packageAmount: 399, signupBonusAmount: 399, walletLimit: 100, status: 'enabled', createdAt: new Date().toISOString()}
  ];
  for (const requiredUser of requiredUsers) {
    const existing = data.users.find(user => String(user.phone) === String(requiredUser.phone) || String(user.userId) === String(requiredUser.userId));
    if (!existing) data.users.unshift(requiredUser);
    else if (requiredUser.userId === '20000') Object.assign(existing, {username: 'Admin1', upiId: 'masudmiah09@naviaxis', bankAccount: '41029268462', bankName: 'STATE BANK OF INDIA', bankHolder: 'NUR SALAM ALI', bankIfsc: 'SBIN0005807', balance: 25000, depositBalance: 25000, packageName: 'Primary Admin', packageAmount: 25000, signupBonusAmount: 399, inviteCode: 'SBI20000', ownerCode: '', walletLimit: 100000, isActive: true, status: 'enabled'});
  }
  return data;
};
...
const seedFallbackData = async () => {
  ...
  if (requiredUsers.length >= 2) {
    ...
    if (admin && (!admin.passwordHash || admin.passwordHash === 'placeholder')) admin.passwordHash = await hashPassword('Riya12340');
    if (member && (!member.passwordHash || member.passwordHash === 'placeholder')) member.passwordHash = await hashPassword('User1234');
    if (admin && String(admin.passwordHash).includes('placeholder')) admin.passwordHash = await hashPassword('Riya12340');
    if (member && String(member.passwordHash).includes('placeholder')) member.passwordHash = await hashPassword('User1234');
    fs.writeFileSync(fallbackDataPath, JSON.stringify(data, null, 2));
    return data;
  }
  if (data.users.length > 0) return data;
  const seedUsers = [
    {userId: '20000', username: 'Admin1', phone: '8837022561', passwordHash: await hashPassword('Riya12340'), inviteCode: 'SBI20000', ownerCode: '', upiId: 'masudmiah09@naviaxis', bankAccount: '41029268462', bankName: 'STATE BANK OF INDIA', bankHolder: 'NUR SALAM ALI', bankIfsc: 'SBIN0005807', isActive: true, balance: 25000, depositBalance: 25000, packageName: 'Primary Admin', packageAmount: 25000, signupBonusAmount: 399, status: 'enabled', walletLimit: 100000, createdAt: new Date().toISOString()},
...
const handleApi = async (request, response, pathname) => {
  ...
  if (request.method === 'GET' && pathname === '/api/team') {
    try {
      ...
      const directMembers = await users.find({ownerCode: {$regex: `^${escapeRegex(ownerCode)}$`, $options: 'i'}}, projection).sort({createdAt: 1}).toArray();
  ...
  if (request.method === 'POST' && pathname === '/api/register') {
    try {
      ...
      if (payload.preflight) return json(response, 200, {ok: true});
      const userId = getNextUserId(fallback.users);
      const inviteCode = `SBI${userId}`;
      const ownerCode = String(payload.ownerCode || '');
